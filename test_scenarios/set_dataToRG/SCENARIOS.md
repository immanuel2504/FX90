# set_dataToRG — test scenarios

| | |
|---|---|
| MQTT command | `set_dataToRG` (publish envelope from `mqtt/`) |
| REST endpoint | `PUT /cloud/setdataToRG` (send body from `rest/`) |
...
| 07 | `mqtt/07_mqtt_wrong_command_name.json` *(MQTT only)* | **REJECTED / NO RESPONSE** | Command name 'set_dataToRG_TYPO' does not exist. | Like addressing the envelope to a department that does not exist - nobody should pick it up. | failure + field named + command_id echoed; state unchanged (read back) |
