# set_hostname — test scenarios

| | |
|---|---|
| MQTT command | `set_hostname` (publish envelope from `mqtt/`) |
| REST endpoint | `PUT /cloud/hostName` (send body from `rest/`) |
...
| 04 | `mqtt/04_wrong_type_hostname.json` · `rest/04_wrong_type_hostname.json` | **REJECTED** | 'hostname' should be string but a int was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
...
| 08 | `mqtt/08_mqtt_wrong_command_name.json` *(MQTT only)* | **REJECTED / NO RESPONSE** | Command name 'set_hostname_TYPO' does not exist. | Like addressing the envelope to a department that does not exist - nobody should pick it up. | failure + field named + command_id echoed; state unchanged (read back) |
