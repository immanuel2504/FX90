# set_region — test scenarios

| | |
|---|---|
| MQTT command | `set_region` (publish envelope from `mqtt/`) |
| REST endpoint | `PUT /cloud/region` (send body from `rest/`) |
...
| 04 | `mqtt/04_missing_required_country.json` · `rest/04_missing_required_country.json` | **REJECTED** | Required field 'country' removed. Reader must fail with a clear message naming the field. | Like submitting a passport application without writing your name - the office cannot even start processing it and hands it straight back. | failure + field named + command_id echoed; state unchanged (read back) |
| 05 | `mqtt/05_missing_required_standardname.json` · `rest/05_missing_required_standardname.json` | **REJECTED** | Required field 'standardname' removed. Reader must fail with a clear message naming the field. | Like submitting a passport application without writing your name - the office cannot even start processing it and hands it straight back. | failure + field named + command_id echoed; state unchanged (read back) |
| 06 | `mqtt/06_wrong_type_country.json` · `rest/06_wrong_type_country.json` | **REJECTED** | 'country' should be string but a int was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
| 07 | `mqtt/07_wrong_type_standardname.json` · `rest/07_wrong_type_standardname.json` | **REJECTED** | 'standardname' should be string but a int was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
...
| 11 | `mqtt/11_mqtt_wrong_command_name.json` *(MQTT only)* | **REJECTED / NO RESPONSE** | Command name 'set_region_TYPO' does not exist. | Like addressing the envelope to a department that does not exist - nobody should pick it up. | failure + field named + command_id echoed; state unchanged (read back) |
