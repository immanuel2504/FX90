# set_preSelection — test scenarios

| | |
|---|---|
| MQTT command | `set_preSelection` (publish envelope from `mqtt/`) |
| REST endpoint | `PUT /cloud/preSelection` (send body from `rest/`) |
...
| 04 | `mqtt/04_wrong_type_preSelection.json` · `rest/04_wrong_type_preSelection.json` | **REJECTED** | 'preSelection' should be boolean but a str was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
...
| 08 | `mqtt/08_mqtt_wrong_command_name.json` *(MQTT only)* | **REJECTED / NO RESPONSE** | Command name 'set_preSelection_TYPO' does not exist. | Like addressing the envelope to a department that does not exist - nobody should pick it up. | failure + field named + command_id echoed; state unchanged (read back) |
