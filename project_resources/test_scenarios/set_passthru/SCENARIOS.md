# set_passthru — test scenarios

| | |
|---|---|
| MQTT command | `set_passthru` (publish envelope from `mqtt/`) |
| REST endpoint | `PUT /cloud/pass-through` (send body from `rest/`) |
...
| 04 | `mqtt/04_wrong_type_component.json` · `rest/04_wrong_type_component.json` | **REJECTED** | 'component' should be string but a int was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
| 05 | `mqtt/05_wrong_type_payload.json` · `rest/05_wrong_type_payload.json` | **REJECTED** | 'payload' should be string but a int was sent. | Like writing the word 'blue' in the phone-number box - right form, wrong kind of answer in that field. | failure + field named + command_id echoed; state unchanged (read back) |
| 06 | `mqtt/06_invalid_enum_component.json` · `rest/06_invalid_enum_component.json` | **REJECTED** | 'component' must be one of ['RC'] - 'NOT_A_VALID_VALUE' is not on the menu. | Like ordering size 'XXL' in a shop that only stocks S, M and L - the value must come from the fixed menu. | failure + field named + command_id echoed; state unchanged (read back) |
...
| 10 | `mqtt/10_mqtt_wrong_command_name.json` *(MQTT only)* | **REJECTED / NO RESPONSE** | Command name 'set_passthru_TYPO' does not exist. | Like addressing the envelope to a department that does not exist - nobody should pick it up. | failure + field named + command_id echoed; state unchanged (read back) |
