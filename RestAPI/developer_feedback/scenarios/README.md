...
| `get_mode` / `set_mode` | `/cloud/mode` | `type` no `enum`; `environment` missing (vs `Command Schemas.json`) |
...
| `set_updateCertificate` | `/cloud/certificates` | `type` no `enum`; `required`; response no schema |
...
| `set_installUserapp` | `/cloud/apps/install` | `required` fields |
